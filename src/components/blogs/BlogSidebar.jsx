...
// import { caseBriefs } from "../../data/caseBriefsData";
import caseBriefs from "../../data/caseBriefsData2";
export const BlogSidebar = () => {
  
  const { category, slug } = useParams();

  const caseBrief = caseBriefs.find(
    (item) =>
      item.slug === slug &&
      item.category === category
  );

  // Ensure caseBrief is found before rendering tags
  if (!caseBrief) {
    return <p>Case brief not found.</p>;
  }
